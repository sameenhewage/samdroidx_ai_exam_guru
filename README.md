...
- [`docs/v1/08_MANUAL_AGENT_INTERVENTION_WORKFLOW.md`](docs/v1/08_MANUAL_AGENT_INTERVENTION_WORKFLOW.md)
...
Implementation is active across Priority 1 acceptance gates. P0 and P1 are DONE; P2 and P3 have substantial implementation evidence but remain incomplete on representative human-reviewed real-data quality gates. Phases are acceptance gates, not waterfall implementation locks, but the current source-fidelity recovery task pauses downstream feature work and corpus backfill until the five fixed real pages pass. See [`docs/v1/PHASE_TRACKER.md`](docs/v1/PHASE_TRACKER.md) for the authoritative current status and evidence.
...
**View PDF / PDF බලන්න** opens the original inside Exam Guru through an application-owned page-image viewer. Only the selected page is requested; first/previous/next/last controls, direct jumps, zoom, fit-page and Retry do not load the whole PDF or depend on a browser PDF plugin. Source-text and human-reference comparison reuse the same read-only page viewer. **Download original PDF / මුල් PDF ගොනුව බාගන්න** is a separate explicit streaming download, never an automatic fallback for a failed preview.
...
Per-page candidates preserve raw UTF-8 evidence, a separate NFC text view, engine/configuration versions and append-only review history. **No NFKC source rewrite, automatic legacy-font conversion, model-authored trusted source truth or bulk trust is permitted.** Corrections create unverified child candidates; confirmation requires explicit comparison with the original and the exact current candidate/version. Rereads preserve prior candidates and human edits. The existing text-lineage path requires current verified page lineage, an exact nonblank NFC source span, admitted metadata/scope and applicable review/freshness checks through generation and publication. Old published versions remain immutable history; a legacy `trusted` flag does not authorize new work.

**Visual document-understanding re-architecture is in progress.** The [authoritative contract](docs/SYSTEM_ARCHITECTURE.md#48-visual-document-understanding-and-trusted-educational-knowledge) separates immutable originals, source-faithful observation candidates, educational interpretation and verified page knowledge. Native/OCR/layout checks remain evidence; only current TrustedPageKnowledge will create the new KnowledgeUnits and deterministic retrieval projections. Strict candidate/verification/rendered-input contracts and a separate PostgreSQL persistence migration are tested in disposable databases. The initial OpenAI adapter has bounded candidate-only transport and durable one-page job/API wiring, disabled by default; the isolated test runner uses an explicitly scoped synthetic provider. Image-bound teacher comparison, explicit verification and versioned correction/exclusion separate observed evidence from accepted teaching meaning without rewriting originals or prior decisions. The preserved real Studio is now on 0055 with the independent local Qwen/OpenAI source-consensus path and separate human VerifiedSourceContent gate. The 17 September safety update retained 426 managed artifacts and every database table inventory without another migration. **Source Fidelity Gate: FAIL**: OpenAI credit exhaustion and inaccurate local Sinhala readings still block the fixed acceptance set; all workers are paused and no verified source, knowledge units or embeddings were created. Broader legacy source/education integration tests and the later educational-analysis flow also remain unfinished. Provider output cannot approve itself, and the source-only real-page gate must pass before downstream work or broad backfill resumes.
