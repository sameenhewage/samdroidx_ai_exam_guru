...
- Frozen read-only backend verification uses `ruff check --no-cache` and `ruff format --check --no-cache`, with mypy/pytest/coverage output in writable verification locations outside source. Before mounting a dependency volume beneath a read-only detached worktree, create only the required ignored empty mountpoint; never make the source writable to bypass a mount error.
