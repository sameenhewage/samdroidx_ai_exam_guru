...
- Existing private env files can retain older legacy OCR limits. Before recreating containers, validate the resolved worker settings without printing secrets: `EXAM_GURU_OCR_TESSERACT_MAX_SOURCE_BYTES` must cover `EXAM_GURU_MAX_UPLOAD_BYTES`, and page/timeout settings must fit the execution budget. Compose's current legacy defaults are 268435456 bytes, 40 pages and five seconds; these are not the separate page-reader budgets. Explicit process-environment overrides persist in the created containers, not in the private env file, so retain them for subsequent Compose recreation commands.
- An empty upload Medium/Subject selector can mean missing grade-linked catalogue data, not a broken browser control. Check the catalogue response and persisted configuration first. Rebuilding or migrating does not populate educational configuration. Do not add fixtures, fabricate catalogue admission or trust sources merely to fill a dropdown; real setup/admission requires the operator's confirmed scope and evidence.
