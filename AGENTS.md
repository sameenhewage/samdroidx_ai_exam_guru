...
- The source-fidelity correction in `docs/SYSTEM_ARCHITECTURE.md` §4.8 governs new work: immutable original/rendered evidence -> source-only multi-pass SourceReadCandidate -> explicit human correction/verification -> immutable VerifiedSourceContent -> original page plus verified source -> separate EducationalAnalysisCandidate -> separately accepted TrustedPageKnowledge -> knowledge/RAG. Stage A must request no educational interpretation. Native/OCR evidence may be compared afterward, never supplied as transcription truth or automatically overwrite the visual result. Provider completion is not source accuracy. Downstream feature work and corpus backfill remain paused until the fixed real-source Chrome MCP gate passes; do not substitute synthetic tests or fabricate human approval.
- Preserve originals, fixed benchmark membership/baselines and every per-page candidate's raw UTF-8 evidence, separate NFC view, source/page hashes, engine/model/configuration versions and append-only review events. Never use NFKC to rewrite source text or silently apply legacy-font mappings. A bounded visual provider may propose source-faithful observations, but cannot author immutable originals, human evaluation references or trusted source truth; verification and authorized decisions remain separate.
...
- Current source eligibility requires `source-fidelity-v2/rules-2/` validation for every candidate method, including human edits. Migration 0040 invalidates older policy results for new use without rewriting their evidence or published history. Legacy-font native output, mixed-script corruption, invalid provenance and unresolved Maths/table evidence cannot be confirmed. Keep original `source_languages` separate from an OCR candidate's observed languages. Readable recovered Unicode may be displayed with a failure warning, never as permission to confirm. Preserve all competing candidates and use versioned selection events rather than overwriting them.
- New knowledge/embedding/RAG use requires the entire original document to be resolved: every page currently verified or explicitly excluded, with at least one verified page. A verified page cannot bypass an unresolved sibling. Use the canonical document/page freshness functions and sibling locks; retain historical published readback and never retrofit trust onto old rows. Bounded compressed mathematical evidence is lossless, versioned and integrity checked; metadata-limit omissions remain failed evidence.
...
- Existing text-lineage paths require current verified candidate lineage and exact nonblank NFC source spans. The new knowledge path must derive only from current TrustedPageKnowledge, retaining exact observed evidence separately from accepted meaning and binding units/projections/embeddings to source and transformation revisions. Both require admitted metadata/scope, applicable record/taxonomy review and freshness checks through generation, validation, approval and publication. Preserve published versions and historical evidence; never retrofit trust onto legacy rows or activate an unverified observation merely to populate RAG.
...
- The active source-consensus task is `prompts/v1/09_SOURCE_FIDELITY_QWEN_OPENAI_CONSENSUS_ENGINE.md`. Keep Qwen and OpenAI independent and pin the local model digest plus Ollama API version. A changed/missing local runtime must stop before paid layout work; exhausted OpenAI credit must not be retried per region. The 17 September inspection found Ollama 0.34.1 with unchanged Qwen model digests and loopback-only listening; reverify current state rather than silently relabeling old witness events. GPU execution, geometry matches and green unit tests are not Sinhala accuracy or human approval.
...
- Python tooling: uv
- document understanding: rendered visual evidence plus replaceable candidate providers; retain native extraction/OCR/layout diagnostics as evidence and benchmark representative real pages before quality claims
...
- The actual private/local Studio and real source fidelity are the primary acceptance environment. GitHub is source control/versioning/collaboration and a secondary regression/security gate, not a runtime dependency. Classify CI failures as caused by the current change, pre-existing, unrelated external infrastructure, or genuine security/data-integrity failures. Fix current-change defects and genuine security/data-integrity failures without weakening checks; document unrelated external/pre-existing failures and continue safe local product work. A successful push is not evidence that remote CI passed.
- Classify every secret-scanner finding before remediation. Never disable scanning, broadly exempt directories or generate random high-entropy substitutes merely to evade detection. Only exact proven non-secrets may use the approved baseline mechanism; preserve detector/filter policy and distinguish location bookkeeping from a new suppression.
...
- On 2026-09-13 the owner explicitly authorized routine environment startup and testing needed to continue work and requested no repeated permission questions for those operations. Start the existing Docker Desktop installation and necessary configured verification services autonomously, then verify engine identity, storage binds and retained data before proceeding. Preserve source data and security controls; this standing permission is not permission for destructive resets, history rewriting or fabricated human approval.
- Verify the current host/runtime rather than assuming a historical Linux/WSL path or shell version. npm gates require the repository-pinned Node 24.19.0 and npm 11.17.0; use verified local/container tooling without relaxing engine requirements. At the 2026-09-10 inspection the checkout was `D:\AI Exam Guru\samdroidx_ai_exam_guru`, and the real source folders were `D:\AI Exam Guru\RAG DATA\Grade 3`, `Grade 4` and `Grade 5`. Recheck paths/engine/binds before operations. Keep new private evidence, model/cache and temporary files on D: or E: where available, outside Git.
- Verify the pinned Playwright/browser executable and cache actually exist before reuse. Chrome DevTools MCP is suitable for interactive inspection when connected; ordinary local Playwright is also available without GitHub. Do not use an IDE/browser preview unless explicitly requested. Synthetic fixtures and destructive acceptance setup must run only through the attested `npm run test:e2e:isolated` runtime. Owner-authorized real-source acceptance uses the actual local Studio through normal application APIs/UI, starting with read-only inspection; never seed fixtures into it, forge human confirmation, or export browser credentials. A cached local browser pass does not prove a fresh installer succeeds.
- Container-hosted Chromium selecting Sinhala/non-ASCII filesystem paths needs a UTF-8 process locale (`LANG=C.UTF-8`, `LC_ALL=C.UTF-8`) and an existing writable `TMPDIR` appropriate to that container. Verify a bare file input can see the actual path; Node filesystem access alone is insufficient. Do not rename originals or buffer a whole PDF to work around a browser-tooling locale failure. Retain source/read-only mounts and application ownership.
...
- Preserve the canonical POSIX paths in `.secrets.baseline`. Native Windows detect-secrets can rewrite path separators and generation metadata; do not commit that platform churn or change finding scope. Use verified POSIX Git/Python tooling for the unchanged full tracked-file hook, compare baseline changes explicitly, and keep Studio Docker commands pinned to the intended engine.
...
- Primary PDF inspection must use the shared application-owned `OriginalPageViewer` and canonical bounded page-image route, never raw PDF iframes/new tabs or browser-plugin assumptions. Mount only the requested image; preserve exact preview identity in review workflows and retain image-failure Retry without downloading. Keep explicit original download separate and clearly labeled. Real browser acceptance must prove no original-PDF requests/downloads during viewing and comparison, plus an intentional byte/hash-matched download of the large source.
- The workspace language is a teacher-presentation default, not OCR or validation authority. Preserve known page language; unread/undetermined pages may use the assigned medium or recognized current candidate/intake label as a UI hint. Never rewrite the page/candidate language, diagnostics, raw evidence, reader configuration or trust to match that hint, and keep explicit browser-local teacher choices authoritative for the UI.
- Candidate-description corrections for unassigned, metadata-required sources are separate append-only `source_metadata_candidates` revisions, not edits to original `intake_metadata`. Keep checksum/scope/candidate versions, actor/reason/audit identity, and draft-version conflict protection. Saving a description cannot admit catalogue scope or verify page text. Explicit later confirmation must bind the current candidate ID before adopting its reviewed year/category; preserve old proposals when scope changes.
...
## Reference-only evaluation and local checkpoint

- Independent human evaluation references are separate from source confirmation. Preserve preview/document/page/checksum/reviewer identity, exact UTF-8 and separate NFC text/hashes, explicit comparison, reason, append-only audit and optimistic reference versions. Never confirm an operational page, clear Maths/table failures, populate legacy `source_page_ground_truth`, admit metadata or authorize RAG from an evaluation reference.
- The teacher editor must start blank or from a prior human reference, never machine output. Keep original-image availability, bounded comparison panes, draft-preserving image retry/conflict recovery and separate evaluation-only counts. A failed latest-version reload must not unlock a stale save.
- On 12 September the owner resumed work. The same Desktop engine/bind restarted with three originals / 376 pages and schema 0041; reference-only migration 0042 had run only in disposable tests. Verify the phase tracker for subsequent release/deployment evidence rather than treating this checkpoint as permission to deploy unfinished work.
- Preserved real-source checkpoint: 1,135 page candidates / 1,642 page events, one metadata candidate, four uploads / 44 parts; 370 failed readings and six needing review, zero verified pages, human references, chunks or embeddings. The genuine 169,816,530-byte / 371-page guide completed its preserved upload and restart-safe processing but remains source-fidelity failed. Real bounded navigation passed pages 1/371/185/56/1 after the presentation fix.
- Private rollout, large-upload, restart and browser evidence remain outside Git under the existing `D:\AI Exam Guru\audit-desktop-initial-20260910-0533`, `private-audit-20260910-92f1` and `private-ui-audit-20260909-owner-local` folders. Reverify the same Docker Desktop engine, existing binds, schema and counts; never switch stores, reset data or replay uploads merely to resume development.
- External formatting can leave an IDE-backed Python buffer different from the file seen by Git/Docker. Verify saved state and test collection, not the editor success message alone. In this Windows session, reading/editing the same file through its extended absolute `\\?\D:\...` path resolved the stale-buffer mismatch; re-run lint/typechecks against the saved file afterward.
- Verify cached frontend installations with `npm ls --workspace apps/web next vitest --depth=0` against the lockfile; a container's historical name is not proof of current dependencies. The web Dockerfile's `dependencies` target provides an exact `npm ci` environment. A root `node_modules` volume alone can omit workspace-nested packages such as `@vitejs/plugin-react`: frozen web verification also mounts `apps/web/node_modules` populated from that same verified dependency image, without borrowing an undeclared host installation. Keep dependency/source mounts read-only and create only the required ignored mountpoints. Read-only-source Vitest can use `--configLoader native --no-cache` with coverage output on a separate writable volume; use `tsc --noEmit --incremental false` to avoid writing build metadata into that source mount.
- Docker Desktop backend verification can use published PostgreSQL/Ryuk ports with `TESTCONTAINERS_HOST_OVERRIDE=host.docker.internal`; `127.0.0.1` was not a valid Ryuk endpoint in this setup. The 0045/0046 bridge-network gates passed, but later mapped-port runs still hit unrelated TCP-connect timeouts with both the hostname and its resolved IPv4 address. For a verifier and disposable service containers on the same Docker bridge, Testcontainers' supported `testcontainers_config.connection_mode_override = ConnectionMode.bridge_ip` with `TESTCONTAINERS_HOST_OVERRIDE` unset uses direct private container addresses/ports. The 0047 full gate and a 200-connection disposable probe passed in that mode. Keep database attestation and Ryuk cleanup enabled; do not switch to the real Studio or relax tests/TLS to avoid connection errors. Browser E2E retains its separately attested host-network/explicit-port profile.
- Original-PDF proxy transfers use separate bounded header, idle-progress and total deadlines; do not apply the ordinary five-second HTTP/OIDC timer to an entire large download or remove timeouts altogether. Retain stream backpressure, abort propagation, timer cleanup and exact byte/hash browser acceptance.
- Frozen read-only backend verification uses `ruff check --no-cache` and `ruff format --check --no-cache`, with mypy/pytest/coverage output in writable verification locations outside source. Before mounting a dependency volume beneath a read-only detached worktree, create only the required ignored empty mountpoint; never make the source writable to bypass a mount error. Formatting is a separate operation in a dedicated container with only the intended checkout writable. The production API image does not itself include Ruff; mount the already-verified development virtualenv read-only rather than assuming the verifier's mounted tools are baked into that image.
- Verify a full Chromium page render, not just executable/version startup. The current E2E browser cache/TMPDIR belong to UID 1000; the established verifier uses `1000:1000`, Docker-socket group 0, a read-only root, dropped capabilities, no-new-privileges and 1 GiB shared memory. A root-launched browser can start yet crash its renderer on that user-owned temporary directory. Match the inspected owner/profile instead of relaxing filesystem permissions or browser safeguards.
- Revalidate a `Settings` snapshot with `model_dump(exclude_unset=True)` when reconstructing it. Dumping every default marks otherwise-unset OIDC fields as explicit and correctly trips the independent identity-configuration guard; do not weaken that guard for an unrelated provider factory.
- Register every new authenticated original-comparison image route in the Next admin proxy's source-image path classifier. Generic proxy forwarding does not apply the image MIME allowlist or private same-origin sandbox headers; test both active-content rejection and valid-image headers for each supported path. Also verify the final response through the running Next server: global `securityHeaderRules` can override route-handler CSP. Private admin `.../image` responses must retain the restrictive source-content rule, not the application HTML policy.
- FastAPI validates parsed Python request containers. JSON arrays need an explicit JSON-mode bridge when nested first-party understanding models require strict tuples; revalidate the first-party model at the boundary rather than relaxing domain strictness. Preserve rejection of boolean-as-integer identities and other malformed fields.
- UUID-derived fixture code segments can accidentally begin with reserved `E2E` markers after a separator. Use a stable alphabetic character directly attached to the random hex segment (for example `G7-C<hex>`), not another separator, and retain tests that explicit `E2E` labels are still rejected. Never retry random generation until catalogue admission happens to pass or weaken the admission guard.
- A deadline around `anyio.to_thread.run_sync(..., abandon_on_cancel=True)` bounds the caller's wait, not the synchronous send; late delivery remains possible. Keep the durable outbox commit before dispatch, fence duplicate/late deliveries, and bound the underlying broker's connect/socket waits and retries. Removing abandonment does not cancel a thread, and moving dispatch before commit can lose work. Dramatiq interruption limits are not OS process-kill guarantees for arbitrary blocking extensions.
- Material indexing intent versions start at zero. Operational readiness comes from the current eligible review and current-configuration vector, not from new-intent existence: earlier authorized vectors may be ready without enrollment, or immediately after an intent is created. A committed embedding job with a missing queue acknowledgement remains recoverable through the existing embedding outbox; do not unlink it or introduce another job/provider call to recover delivery.
- Keep synthetic educational PDF text fixed. Opaque timestamp-derived markers can legitimately trigger conservative encoded-text diagnostics; put test identity in a bounded non-rendered PDF comment before upload, not in visible source text. Preserve checksum uniqueness across disposable scopes, exact original download and extraction assertions, and the original rejected fixtures. Never weaken source-fidelity guards or regenerate random text until it passes.
- Dirty Next.js review drafts require a pre-navigation guard, not a late `popstate` handler after unmount. When Navigation API cancellation is unavailable, the normal entry must preserve native document/unload protection. Test actual Back/Forward cancellation, consented departure and native modifier clicks; a missing Playwright page event alone does not prove a tab failed to open. Inspect the actual same-context target and rendered document rather than creating a replacement tab.
