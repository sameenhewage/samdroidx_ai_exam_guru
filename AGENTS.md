...
- Bridge-networked Docker Desktop backend verification requires `TESTCONTAINERS_HOST_OVERRIDE=host.docker.internal` for mapped PostgreSQL/Ryuk ports. Keep disposable database attestation and cleanup enabled; do not disable Ryuk or switch to the real Studio to avoid a test-container connection error. The 0045 full backend gate passed in this bridge profile after two host-network runs hit unrelated TCP-connect timeouts; a `127.0.0.1` Testcontainers override could not reach Ryuk in this setup. Browser E2E uses its separately attested host-network/explicit-port profile.
