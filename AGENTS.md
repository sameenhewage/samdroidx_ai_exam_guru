...
- Current source eligibility requires `source-fidelity-v2/` validation for every candidate method, including human edits. Legacy-font native output, mixed-script corruption, invalid provenance and unresolved Maths/table evidence cannot be confirmed. Keep original `source_languages` separate from an OCR candidate's observed languages. Readable recovered Unicode may be displayed with a failure warning, never as permission to confirm. Preserve all competing candidates and use versioned selection events rather than overwriting them.
