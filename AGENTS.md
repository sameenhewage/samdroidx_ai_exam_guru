...
- The document-understanding architecture correction in `docs/SYSTEM_ARCHITECTURE.md` §4.8 governs new source-to-RAG work: immutable original/rendered evidence -> candidate source observation -> separate educational understanding -> independent verification -> versioned TrustedPageKnowledge -> KnowledgeUnits -> deterministic projections -> hybrid retrieval -> grounded generation. Native extraction, OCR and mathematical/layout diagnostics remain evidence mechanisms. Do not mistake the new contracts for a completed persistence/provider/RAG cutover.
