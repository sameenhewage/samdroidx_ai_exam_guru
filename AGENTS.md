...
- Bridge-networked Docker Desktop backend verification requires `TESTCONTAINERS_HOST_OVERRIDE=host.docker.internal` for mapped PostgreSQL/Ryuk ports. Keep disposable database attestation and cleanup enabled; do not disable Ryuk or switch to the real Studio to avoid a test-container connection error.
