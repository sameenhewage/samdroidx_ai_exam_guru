...
- Remaining repository hardening requires live production identity acceptance, human Sinhala OCR/data review, representative P3-P5 real-data thresholds and a configured/evaluated factual semantic verifier.
...
# Current next action
P2 remains IN_PROGRESS on human-adjudicated Sinhala OCR ground truth. P3-P5 remain IN_PROGRESS on representative human-reviewed real Grade 5 data and measured quality thresholds, and live production identity acceptance remains externally blocked. The next non-blocked Priority 1 slice is a provider-independent, schema-constrained factual semantic-verifier adapter with deterministic fake-provider evals, bounded private-source context and an opt-in live quality/cost baseline. No production factual-quality claim may be made until a real provider configuration and representative human-adjudicated evidence are available. Student Priority 2 remains blocked by P10.
