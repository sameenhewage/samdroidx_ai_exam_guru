...
- `docs/v1/06_KNOWN_LIMITATIONS.md` records the unresolved human-data, paid-model, live-IdP and deployment responsibilities without waiving any gate. The generic OIDC implementation is complete, but the production identity criterion remains open until a real external tenant/client/role mapping and browser session are exercised.
- Remaining repository hardening includes live production identity acceptance, final security/adversarial acceptance and green release-commit CI evidence. External blockers remain human Sinhala OCR/data review, representative P3-P5 real-data thresholds and the executed paid P8 live baseline.
