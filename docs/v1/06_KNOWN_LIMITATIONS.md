...
- Factual/language subjects now have an optional strict OpenAI semantic-verifier adapter with schema-constrained supported/contradicted/insufficient-evidence output, exact source/page reference confinement, provider/model/prompt/pricing lineage, byte/token/cost/latency bounds and deterministic mocked coverage. This is a verification aid, not proof by model agreement.
