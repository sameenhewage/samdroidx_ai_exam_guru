...
- Metadata candidates from checksum-bound download manifests and document inspection are display-only, not RAG authority. Three unique sources have conflicting Grade 4/5 evidence and appear as unassigned. Explicit metadata confirmation requires an actual active curriculum assignment. Font-risk and finalized pending-OCR resolution is not automatic; a reviewed, provenance-preserving adjudication workflow is still needed before these sources can become trusted.
