...
- The 2026-09-05 intake imported all 587 unique PDFs from the 696 Grade 3/4/5 PDF paths into the current normal Studio, preserving all 711 original/evidence files unchanged. All imported sources require metadata and text review; none was trusted, chunked or embedded. Earlier reviewed Grade 4/5 sample evidence belongs to a separate pilot runtime, not to these new imports.
- Native/OCR processing produced reviewable extraction for 574 originals. Thirteen originals remain failed with `unsafe_text` instead of silently stripping control/glyph corruption; 64 extracted documents retain unresolved OCR/empty-page warnings. Font/private-use/replacement-glyph checks flag 251 extracted originals conservatively, not as proof that every flagged page is wrong. These categories overlap and do not establish educational quality.
- Metadata candidates from checksum-bound download manifests and document inspection are display-only, not RAG authority. Three unique sources have conflicting Grade 4/5 evidence and appear as unassigned. Explicit metadata confirmation requires an actual active curriculum assignment. A missing source year is adopted from intake only on that explicit confirmation, without overwriting an existing year; pending year display/filtering does not promote it. Font-risk and finalized pending-OCR resolution is not automatic; a reviewed, provenance-preserving adjudication workflow is still needed before these sources can become trusted.
- The intake API rejects non-printable Unicode and padded labels, including non-breaking spaces. The independent PostgreSQL guard bounds JSON, types, lengths, ordinary whitespace and control characters, but does not reproduce every Python Unicode classification for privileged direct SQL writes. PostgreSQL's current locale treats non-breaking space as printable and not whitespace, so a generic POSIX-space/printable regex substitution is not a complete fix. This defense-in-depth gap remains documented; direct SQL is not a supported intake path, and no trust/audit guard is relaxed.
- In the earlier pilot runtime, the Grade 3 Sinhala Maths extraction was removed from active use after visual comparison exposed legacy-font glyph corruption despite non-empty extracted Unicode text. Its newly imported current-Studio original remains visible for review but cannot be trusted; it has not been deleted or marked removed. Reprocessing the exact 7-page source (`a5678c4...c6058`) in the current runtime reproduced 4,917 native characters with `needs_ocr=false` and zero OCR pages, so installing Tesseract alone does not repair this case. Grade 3 requires a corrected font mapping or an explicit human-adjudicated OCR/correction path before it can re-enter retrieval.
