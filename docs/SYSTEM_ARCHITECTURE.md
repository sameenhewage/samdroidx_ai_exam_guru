...
PDF / image -> immutable original -> rendered page / regions
            -> independent Qwen local + OpenAI source-only readings
            -> deterministic alignment + targeted higher-DPI rereads
            -> MachineSourceCandidate + independent quality checks
            -> explicit human correction / verification
            -> immutable, versioned VerifiedSourceContent
            -> ORIGINAL PAGE + VERIFIED SOURCE -> EducationalAnalysisCandidate
            -> separately accepted, versioned TrustedPageKnowledge
...
Keep five distinct layers, in this mandatory order:
...
2. **Source reading:** `SourceReadCandidate` contains only visible Unicode text, headings, paragraphs, reading order, literal numbers/operators, equations, vertical arithmetic, complete table/grid positions, blank/unreadable cells, URLs/emails, objects/groups, labels and visible relationships. Stage A requests no topic, skill, objective, educational meaning, curriculum interpretation, answer or teaching explanation. A provider/job completion is not a source-fidelity pass.
3. **Human-verified source:** only an authorized explicit original comparison, correction where needed and confirmation creates immutable `VerifiedSourceContent`. The exact source/candidate/image/review revision and raw Unicode survive reload. Unreadable cells, missing text regions and unresolved blocking findings cannot be cleared by provider success. New readings, corrections, exclusion/reopening and source changes invalidate active verification without rewriting history.
4. **Educational analysis:** a separate explicit later request receives the **original rendered page plus current VerifiedSourceContent**. Its `EducationalAnalysisCandidate` may propose topics, concepts, skills, objectives, activities, educational relationships and curriculum candidates. It cannot replace verified source observations or authorize itself. Creation, dispatch and completion must recheck the verified revision.
5. **Trusted page knowledge:** only after the verified-source and later-analysis gates may separately accepted understanding be combined with exact source observations as `TrustedPageKnowledge`. No verified source means no educational analysis, knowledge preparation, embeddings, RAG or generation. Published history remains immutable; no legacy candidate or old combined observation/interpretation run is promoted by migration.

**Source-fidelity recovery lock:** downstream feature development and corpus backfill are paused. First prove the fixed cover, flower-multiplication, grid, vertical-arithmetic/table and dense-prose pages in the real localhost Studio through Chrome DevTools MCP. Use Sol first as the visual-quality baseline; compare other available models only afterward. Synthetic tests prove mechanisms, not real Sinhala accuracy. Exact numbers, operators, URLs/emails and known table cells are required, and any known unflagged source error means the gate is FAIL.

Stage A uses a bounded whole-page layout pass, independent original-resolution region transcription, dedicated detail crops and deterministic reconstruction. Uncertainty is reread only within the affected region and bounded request/token/time/cost limits. Preserve the exact original PNG for the whole-page pass and parent/crop hashes, coordinates, prompts, responses, structural errors and known accounting for every provider pass. Native extraction and Tesseract are secondary evidence compared after visual extraction; neither supplies transcription truth to the visual prompt nor overwrites its output.
...
Teacher review reuses the app-owned original-page viewer with **SYSTEM-READ SOURCE CONTENT** on the right and the original on the left. Confirm, correct, request a reread and exclude remain source actions. Educational meaning is hidden until source verification and belongs to a separate later analysis panel. Original-image failure blocks comparison decisions. Corrections and conflicts preserve drafts and create new candidate revisions; UUIDs, hashes, raw JSON and provider accounting stay under technical details.

The active recovery task is `prompts/v1/09_SOURCE_FIDELITY_QWEN_OPENAI_CONSENSUS_ENGINE.md`. The current `qwen-openai-source-consensus.v1` adapter uses a 300-DPI layout/geometry frame, independently reads the same exact 400-DPI source crops with Qwen and OpenAI, and reserves bounded capacity for 600-DPI disagreement recovery. Neither reader receives the other's text. Raw Unicode and line boundaries remain source evidence; canonical matching does not rewrite the retained text or correct printed arithmetic. Raster table boundaries determine every physical cell coordinate and explicit blank/unreadable state before reading. Model-proposed table reconstruction is not authoritative. Geometry `source-raster-geometry.v2` suppresses narrow grid-edge and tiny intersection artifacts while retaining detached source marks; the fixed grid's 300-DPI blank counts match visual inspection, but higher-DPI geometry and decorative-cover segmentation still have limitations. Row/column **labels**, such as row label 5 and column label 10, are not physical array indices.

The local reader requires the configured `qwen3-vl:8b` model digest and explicit `EXAM_GURU_SOURCE_QWEN_API_VERSION`. It checks `/api/version` and `/api/tags` before paid layout work and again before each local image request; missing/changed local runtime identity blocks further work, retaining already known OpenAI accounting. The transport disables redirects and environment-proxy routing and accepts only approved local endpoints. The Windows default remains `127.0.0.1:11434`. Docker Desktop may use the explicitly enabled `host.docker.internal` alias to the same loopback-bound host service; this does not authorize an Ollama LAN listener. Context/output defaults are 8,192/4,096 tokens, with the supported thinking parser, seed 23 and temperature 0.3. These settings establish reproducibility, not Sinhala accuracy. OpenAI `credit_balance_exhausted` and `insufficient_quota` stop the page instead of being retried once per region.

Migration **0054** adds append-only source-reading events and human `verified_source_contents`, plus separate educational-analysis persistence scaffolding. New source correction/confirmation payloads contain no educational interpretation. Source verification requires the current candidate/report/image/review revision, explicit original comparison and complete region/uncertainty decisions. It does not create TrustedPageKnowledge. New trusted knowledge and legacy source-lineage eligibility require current verified source and later educational analysis; historical evidence is retained rather than promoted. The complete Stage B operator/service flow and reconciliation of legacy combined-stage integration fixtures are unfinished, so this is not a full downstream cutover or a green full-backend acceptance claim.

Migration **0055** adds immutable `source_witness_events` and `source_machine_candidates`. Reader/configuration, source/page, render/crop, request/completion, audit and lease identities are retained independently. Candidate readback revalidates selected text and table values against the bound witnesses and geometry. Unresolved, malformed, stale or missing evidence stays `needs_attention`; `machine_ready` is still not human verification. Paginated witness reads and exact recorded-image reads require source-read authorization. Declared missing/corrupt render artifacts fail closed rather than being regenerated. Both the admin proxy and Next response-header rules retain private/no-store, same-origin, nosniff and sandbox controls for source image responses. Cross-job provider-cache reuse and complete real-source acceptance are not yet proven.
...
The normal Materials **Review page content** screen now reuses the app-owned original-page viewer and separates observed evidence, proposed teaching meaning and uncertainty. Its private candidate-image endpoint checks the immutable original, declared artifact and candidate image hash; verification repeats these availability/identity checks on the server before recording the existing versioned decision. The teacher explicitly confirms original comparison and every observed region, selects accepted teaching claims, resolves recorded uncertainties and supplies a reason. Verification cannot be authorized by a client-supplied trusted flag. Missing/substituted image evidence, blocked findings, stale versions and insufficient permission remain failures. Image errors preserve review choices but clear comparison consent; rebase requires an explicit latest-reading choice. The checked view displays only the accepted meaning in `TrustedPageKnowledge`, with unaccepted provider proposals and technical metadata kept separate. Neither page comparison nor metadata admission bypasses the whole-document readiness gate.

Migration **0045** adds an immutable parent-candidate relationship for explicit human corrections without changing the v1 candidate fingerprint shape or rewriting existing candidates. A correction binds the current page version, exact parent/source/image, full content fingerprint, actor, reason and request identity; original-image verification runs before persistence. Corrected observations/teaching meaning are new unverified candidates with independent checks and fresh region identities. Active trusted pointers are cleared, while earlier candidates, decisions and trusted snapshots remain intact. A shared PostgreSQL transaction-scoped request-identity fence serializes analysis-job and run insertion, so concurrent paid analysis and human correction cannot claim the same owner request ID or misassociate an eventual result. Human runs carry no fabricated provider configuration or accounting.

Explicit exclusion and reopening reuse immutable page audit events, source checksums and optimistic page versions. Exclusion is not verification and cannot overwrite an active analysis. It preserves candidate/report history and clears active trust. Reopening never restores a prior trusted pointer: unread pages return to unprocessed, blocked observations still require correction/reprocessing, and valid observations return to fresh human review without forcing another paid read. Reads validate the exact exclusion event and strict numeric identities. PostgreSQL separately fences excluded-page transitions; a downgrade refuses to drop correction lineage or review-lifecycle protections while corresponding history exists.

The teacher editor changes existing source text, literal equations, cell values/explicit blank-or-unreadable states, visible counts/printed totals and proposed teaching points with source links. It preserves recorded geometry, relationships and uncertainties, rather than silently resegmenting or resolving them; arbitrary region/layout reconstruction is not part of this editor. Drafts remain version-bound through conflicts, and an explicit correction rebase retains the edits but requires the new parent image. Only the JSON correction route receives a bounded **1 MiB content envelope plus 64 KiB transport allowance** in the browser proxy. Other JSON, legacy multipart and resumable chunk limits are unchanged. The API bridges JSON arrays into the strict first-party contract without relaxing domain validation; saving a correction does not confirm it.

The initial pure `knowledge/units.py` contract derives deterministic source components only from a revalidated `TrustedPageKnowledge` snapshot. Parent/semantic region links, accepted cross-region teaching claims and resolved cross-region uncertainty keep their evidence together; reading-next links alone do not merge unrelated components. Units retain exact structured observations, accepted meaning, resolved uncertainty, region identities and versioned source/curriculum/admission metadata. Their deterministic identifiers change with the trusted snapshot or scope rather than rebinding old data. A projection is a separate NFC rendering for retrieval, retaining literal equations, explicit blank cells and distinct observed counts/printed totals; it is never the unit's source truth. Version 1 fails rather than truncates above 32,768 characters / 65,536 UTF-8 bytes, and does not invent searchable content for a purely decorative component. These pure models do not establish live admission/currentness or authorize indexing; repository/SQL freshness and whole-document checks remain necessary.

Migration **0046** persists `knowledge_units`, exact `knowledge_unit_regions` links and `knowledge_projections` without writing legacy chunks, embeddings or source trust. Preparation requires the exact current trusted-page ID, current admitted source scope, every original page currently verified or explicitly excluded, and at least one verified page. A document-scoped derivation mutex plus source, sibling-page, catalogue/curriculum and optional unit/lesson locks prevent duplicate preparation and concurrent scope/review changes from escaping the checks. Python and PostgreSQL independently reproduce the same components and deterministic projection; append-only audit events bind the actor, source/page, trusted version, scope fingerprint and result identities. Missing region links, invented source/projection content even with recomputed hashes, and mismatched audit evidence reject the transaction. Readback revalidates every payload-derived column, not only its JSON hash.

Persistent scope uses **`knowledge-scope.v2`**, adding literal material type/year/paper identity to the original pure-contract bindings. Once derived knowledge exists, source reclassification/descriptor rewriting is rejected in the application and database; audited removal from use remains available. Stale, excluded, unresolved, removed or admission-revoked evidence becomes ineligible without deleting its units/projections. Purely decorative components keep their evidence but receive no invented searchable text. Persistence is idempotent for unchanged inputs and rejects incomplete/conflicting replay evidence rather than silently repairing history. Downgrade refuses retained derived knowledge. The pinned PostgreSQL/pgvector image includes **`uuid-ossp` 1.1**; the migration installs its standard UUIDv5 function for database/Python identity parity and intentionally does not drop that shared extension on downgrade.

Migration **0047** adds append-only `knowledge_unit_reviews` for explicit educational/curriculum classification, separate from both page-source verification and deterministic preparation. Only the latest versioned, positively confirmed mapping to active reviewed taxonomy can satisfy a projection's record-review eligibility predicate. Same-curriculum unit/lesson refinement is allowed where the source was unassigned; an existing approved source unit/lesson cannot be widened or replaced. New approval requires current source knowledge and scope, while explicit withdrawal remains available for stale evidence. Rejected decisions carry no approved classification. Source and taxonomy locks, optimistic review versions, exact audit payloads and immutable historical readback protect this boundary; downgrade refuses retained review history.

Private curriculum-scoped APIs read one unit/review workspace and append a review decision. They verify the requested curriculum on both read and write, preserve private/no-store errors, accept wire JSON through an explicit strict first-party bridge and do not start preparation or indexing. The generated TypeScript client represents the new contracts. A reviewed mapping is not a source confirmation, vector-generation result or publication approval.

Preparation and unit-review services additionally support explicit caller-owned transactions through `commit=False`, without changing standalone defaults or authorization. Successful new writes are flushed; idempotent replay does not commit unrelated caller work. Domain errors leave completion to the caller, while database errors still require caller recovery and deferred constraints remain enforced at commit. No savepoint, background job, automatic classification or indexing is implicitly created by this transaction option. This is a composition prerequisite for bounded workers and explicit review groups, not permission to prepare an entire guide synchronously inside a source-verification request.

Migration **0052** adds explicit durable `material_knowledge_requests` and one-page `knowledge_preparation_jobs`. Normal Materials verification/exclusion injects a typed recorder before the source transaction commits, binding enrollment to the exact document/checksum, actor and source audit. The HTTP transaction does not derive content or send a broker message. Historical direct-service fixtures may omit the recorder; migration creates no enrollment or jobs for old sources. Independent metadata confirmation/catalogue admission may follow page decisions. Recovery discovers at most eight currently eligible pages among enrolled documents per pass, using the existing whole-document and admitted-scope guards; exclusions do not become verified knowledge. Immutable TPK/scope/transformation identities, source-before-job locks, unique input keys, matching audits and terminal-history guards protect the ledger. Downgrade refuses retained intent/jobs.

The existing Dramatiq/Valkey worker prepares one page per delivery and atomically commits units/projections/audit plus job success. Temporary unavailability defers the same input without spending preparation attempts; changed TPK/scope supersedes it. Actual completion/failure and conservatively expired eligible claims consume at most three attempts. Duplicate or late messages are safe through row/lease/version fencing, and the durable queued state commits before dispatch. Recovery uses 90-second SQL statement limits, 10-second source-lock/caller-dispatch waits, zero broker retries and a separately owned publisher with five-second connect/socket timeouts and zero transport retries. A caller dispatch timeout does not prove non-delivery or cancel a synchronous thread; recovery retains ambiguous queued work and closes its publisher. Preparation has a 90-second cooperative execution scope, 120-second Dramatiq interruption limit and default 300-second lease (bounded to 121–3,600 seconds). These are not OS process-kill guarantees for arbitrary blocking extensions. The default five-second outbox age can defer fresh dispatch until the next scheduler tick.

The private, read-only Materials preparation summary checks source and knowledge read permissions before one aggregate query, counts only current exact TPK/scope preparation and does not enroll or dispatch work. The teacher panel uses the generated client, inherited English/Sinhala presentation, cancellable serial status polling, safe errors and no required technical identifiers/actions. `prepared` requires consistent completed-page counts and explicitly does not imply curriculum approval, embeddings, generation readiness or publication. Decorative content can have zero searchable projections. Historical prepared rows do not imply enrollment in the new automatic workflow.

Migration **0053** adds `material_knowledge_index_intents` for the separate material-owned curriculum-review workflow. A new positive per-unit decision uses the existing review service with caller-owned completion and commits the review, exact source/review/projection intent and audits in one transaction. Provider configuration is not needed to save this decision. Existing technical review APIs and 0052 enrollment do not implicitly authorize indexing. Rejections create no intent, newer reviews supersede earlier authority, and decorative units retain a non-searchable outcome. Input identity is immutable; operational state/version and per-attempt bindings have exact append-only audit chains. No source or review history is backfilled, and downgrade refuses retained indexing intent.

Bounded recovery persists a fixed dispatch key and full secrets-free embedding configuration before calling the existing `EmbeddingJobService.create`; it does not bypass that service's source, provider, budget or retry checks. Recovery first reconciles any job already committed under the saved key, even if active configuration has changed. Source-before-intent locks, CAS and stopped-state fences prevent a paused coordinator from reviving withdrawn or changed work. Configuration changes cannot silently allocate another paid job. Explicit retry preserves the human review, requires confirmation/reason/version, retains prior binding audit, caps initial dispatch plus three retries across configuration changes and also respects the existing provider retry depth. Failed/ambiguous work is not automatically retried. The publisher uses bounded connection/socket waits and no transport retries; commit-before-dispatch and ambiguous-delivery handling remain unchanged.

The new automatic indexing path refuses implicit local/deterministic fallback outside an explicitly attested test runtime. It reuses existing provider factories without enabling configuration or calling a live provider during normal tests. Read-only material unit lists/workspaces are document-owned and paginated; readiness requires current source, an eligible review and an actual vector in the current approved configuration, not an old job status or the existence of a new intent. Previously authorized current vectors may be ready without new enrollment, and intent version zero is valid. Teacher review uses source-page/section navigation, readable curriculum choices, original/structured evidence, preserved conflict drafts and separate indexing retry. Technical identities remain hidden, and none of these states bypass generated-question validation or human publication.

Migration **0048** adds reviewed projection targets to the existing embedding tables, durable queue and provider interfaces rather than creating another indexing system. Embedding rows retain exactly one legacy-question, legacy-chunk or projection target, with immutable vectors, exact projection text hashes and configuration-specific deduplication. Job targets are immutable and share the existing total/provider batch limits, leases, retry lineage and permissions. Versioned projection-source references capture trusted-page/unit/projection/review identities and fingerprints in the job-creation audit and source fingerprint. Source/sibling/taxonomy locks plus the exact expected review identity protect dispatch and storage; an intervening positive review is not a silent rebind, and withdrawn/unreviewed sources cannot trigger a provider call. Completed vectors are reused during an explicit partial-work retry. Stored-vector audits retain reported provider accounting when supplied, not invented zero usage.

The existing embedding API/client accepts projection IDs as a third server-owned target category. Legacy technical UI selections explicitly send an empty projection list without exposing new teacher choices. Forward/empty-downgrade/reapply preserves legacy vectors, jobs, source fingerprints and evidence; downgrade refuses retained projection jobs/vectors. The legacy insertion-integrity trigger name remains attached to legacy inserts while a separate unconditional trigger rejects vector update/delete. Older-schema fixtures use only columns present at their original revision. No runtime migration activates the real corpus, provider settings or normal teacher indexing automatically.

The existing hybrid repository now includes reviewed current projections in both lexical and configured-vector candidate sets, with grade/exam/medium/subject/curriculum/unit/lesson/taxonomy filters applied before ranking. Versioned projection references retain the exact trusted-page, unit, projection and review identities/fingerprints through fusion and provenance responses. Separate structured units are not merged merely because their rendered retrieval text matches. Legacy ranking and legacy source identities are retained.

Migration **0049** extends the existing immutable generation context snapshot, not the source-of-truth or a separate generation queue. `generation-knowledge-context.v1` binds selected projection IDs to full `knowledge-evidence.v1` bundles containing exact observations, accepted meaning and source/region/transformation lineage. Generation and validation reconstruct and revalidate that same evidence; plain retrieval text is not substituted for it. Full bundle size counts toward existing generation limits, and `semantic-knowledge-context.v1` includes the same bounded source data in independent semantic verification. These are versioned data envelopes under unchanged trusted instructions, not new model authority. Legacy payload/fingerprint shapes omit absent evidence so historical results remain reproducible.

Source, sibling, scope and exact review freshness are checked before generation and at completion, validation, review transitions and new publication. PostgreSQL compares the persisted evidence, projection text, reference identity and approved scope against current authoritative records. A withdrawal during a model call rejects the candidate but retains known accounting; published readback and failed/older history are preserved. Forward/empty-downgrade/reapply retains legacy publication evidence; downgrade refuses any structured-generation history. The existing teacher orchestration selects projection references internally and preserves them through explicit regeneration/retry. Structured cross-curriculum generation additionally requires the versioned reviewed-programme binding below; a policy label is not authority. Existing legacy programme context shapes are unchanged.

Migration **0050** adds `knowledge-programme-binding.v1` to structured programme runs under `generation-knowledge-context.v2`. The server derives the reviewed policy ID/content hash and exact selected mapping IDs from existing teacher programme orchestration; ordinary clients do not choose them. PostgreSQL verifies the retained policy/review audit, complete mapping membership, target slot/part/lesson/taxonomy, canonical identities and exact allowed source-scope set. It recomputes the descriptive policy label rather than accepting it as authority. Source records retain their actual supporting-grade curriculum scope: existing lineage/evidence predicates are reused per record, not replaced with a target-grade label.

The current admitted target and every declared source scope must remain active and reviewed, including unused scopes in the selected policy set. Policy, catalogue, lesson/unit and taxonomy locks protect transactional completion, validation, review and new publication; retirement or scope withdrawal invalidates new use while published readback remains unchanged. Downgrade refuses retained programme-bound history, but legacy and same-curriculum v1 histories remain eligible for a preserving empty-programme downgrade. Regeneration and explicit retry retain the same binding and full context. Programme feedback derives subject identity from the server-resolved job snapshot rather than requiring an optional subject field in teacher intent.

Programme evidence-availability checks reuse the same hard-scoped record and embedding-target queries as hybrid retrieval. They include current reviewed projections and legacy records with canonical verified lineage, not old document-trust flags or retained vectors alone. Ancestor taxonomy filters have the same descendant semantics as RAG, and grade/exam/medium/subject mismatches cannot appear available merely because a curriculum ID matches. These checks establish eligibility, not live model quality or active-configuration vector readiness.

Migration **0051** adds preservation checks for programme evaluation replay. New programme feedback carries `programme-replay-evidence.v1`: the exact generation identity, policy binding and retained reviewed-policy snapshot, target slot/medium, retrieval filters and source scopes. PostgreSQL binds this envelope to the original generation/validation records and rejects substituted, omitted or self-rehashed policy/source data. The pure replay parser derives allowed supporting-grade bindings from this evidence rather than trusting a `programme_authorized` flag or policy label; grounding evidence retains its real source scope. Historical replay may use retained retired-policy evidence, but cannot authorize fresh generation, source use or publication.

Evaluation runner v2 and private export v2 preserve full structured grounding evidence, generated/source scope bindings, programme evidence and generation lineage. Stored v1 runner readback remains v1; old snapshots are not rewritten or silently granted programme authority. Export remains a permission-checked private Studio evaluation workflow, separate from student publication and from automatic training. Downgrade refuses retained programme-replay or v2-run history. Existing legacy snapshots without a bound programme envelope keep their original fail-closed cross-curriculum behavior.

**Current implementation boundary:** first-party structured candidate, verification/snapshot, rendered-input and PostgreSQL persistence contracts, candidate-only OpenAI transport, bounded job/API wiring, image-bound teacher comparison, versioned correction/exclusion, source-bound unit/projection persistence, explicit unit-review APIs, projection embeddings, scoped hybrid retrieval, bounded same-curriculum generation/validation/publication, reviewed-programme binding, policy-preserving evaluation replay, bounded automatic Materials preparation/progress and material-owned curriculum review with durable indexing intent have been added. The normal curriculum-review journey has synthetic browser evidence, including draft-safe navigation and separate indexing readiness. Real-source deployment/cutover and controlled educational-quality acceptance remain pending; a prepared/reviewed/indexed label alone cannot establish current source, review and vector eligibility. The preserved real Studio is now on migration `0055_source_consensus`, without automatic source trust or backfill. The 17 September local safety rollout preserved all 426 managed artifacts and all 80 table inventories, with no schema change in that rollout; the earlier 425-artifact checkpoint is unchanged and the additional artifact is a retained render. Source reading is configured with `gpt-5.6-sol` and the pinned local Qwen model, but workers are paused following evidenced OpenAI credit exhaustion and continuing Sinhala/model failures. Generation, semantic-verification and embedding providers are not newly enabled. All five fixed acceptance pages remain source-fidelity failed and no VerifiedSourceContent, knowledge units or embeddings have been created. Unit/source-persistence/browser mechanics are not a real Sinhala quality pass; broader combined-stage regression reconciliation and source/education cutover also remain unfinished. The fixed real-source Chrome gate must pass before broad Grade 3/4/5 backfill or downstream feature work resumes.
