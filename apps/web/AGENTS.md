...
- The Source V2 page-review screen (`src/components/admin/source-v2-review.tsx`) follows the language contract locked as **D21** in `docs/source-v2/DECISIONS.md`: domain labels and statuses are bilingual Sinhala + English where the term matters, action controls use concise English, explanatory text follows the page's language Sinhala-first, and original source content is never translated. Do not revert that screen to Sinhala-only action labels such as `පෙළ නිවැරදියි`, `පෙළ නිවැරදි කරන්න` or `වර්ගය වෙනස් කරන්න`; the invariants below that name those strings describe the earlier source-read review screen, and D21 governs Source V2. Human correction on that screen is an unbounded revision chain (**D20**): the editor always opens on the text the card is showing, a control that would destroy a human decision is withheld rather than disabled, and whether a region carries printed text is read off the text, never off `source_kind`.
