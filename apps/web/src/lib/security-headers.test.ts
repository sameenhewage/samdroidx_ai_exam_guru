...
describe("web security response headers", () => {
  ...
  it.each(
    (["local", "test", "staging", "production"] as const).flatMap(
      (appEnvironment) =>
        ["development", "production", "test", undefined].map(
          (nodeEnvironment) => ({
            appEnvironment,
            nodeEnvironment,
          }),
    ...
    "isolates eval permission for APP_ENVIRONMENT=$appEnvironment NODE_ENV=$nodeEnvironment",
    async ({ appEnvironment, nodeEnvironment }) => {
