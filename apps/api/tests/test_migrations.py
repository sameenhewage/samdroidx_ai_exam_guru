...
def test_source_v2_description_is_the_single_bounded_revision_head() -> None:
    config = Config(str(Path(__file__).parents[1] / "alembic.ini"))
    scripts = ScriptDirectory.from_config(config)
    assert scripts.get_heads() == ["0061_source_v2_description"]
    for identifier, parent in (
        ("0061_source_v2_description", "0060_source_v2_single_reader"),
