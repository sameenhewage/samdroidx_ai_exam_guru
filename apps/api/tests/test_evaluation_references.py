...
from exam_guru_api.documents.fidelity_models import SourceEvaluationPreviewModel, SourceEvaluationReferenceModel
from exam_guru_api.documents.fidelity_schemas import EvaluationReferenceSaveRequest
from exam_guru_api.documents.page_images import PageImageArtifacts, PageImageError, SourceImageIdentity, SourceImageStorage
from tests.test_source_fidelity_api import ADMIN, BENCHMARK_ID, DOCUMENT_ID, REVIEWER

PREVIEW_ID = UUID(int=85991)
SOURCE = SourceImageIdentity(DOCUMENT_ID, "a" * 64, f"sources/aa/{'a' * 64}.pdf", 100, 1, "Fixture.pdf")
METADATA: dict[str, object] = {"sha256": "b" * 64, "width": 800, "height": 1200, "page_number": 1, "artifact": {}}


@pytest.mark.parametrize("metadata", [None, {}, {"failure_code": "unavailable"}, {"page_number": 2}])
def test_declared_invalid_comparison_images_are_never_regenerated(metadata: object, monkeypatch: pytest.MonkeyPatch) -> None:
    render = Mock()
    monkeypatch.setattr(module, "render_page_image", render)
    with pytest.raises(PageImageError, match="metadata_invalid"):
        module._capture_image(SOURCE, 1, {"page_image": metadata}, Mock(spec=SourceImageStorage), Mock(spec=PageImageArtifacts))
    render.assert_not_called()


def test_declared_image_is_verified_and_reused_without_rendering(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    provenance: dict[str, object] = {"source_checksum_sha256": SOURCE.checksum_sha256, "page_image": METADATA}
...
def test_service_boundaries_and_responses_without_database_io(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(module, "get_review_workspace", AsyncMock(return_value=SimpleNamespace(document_title="Fixture.pdf", language="si")))
    ...
    preview = SourceEvaluationPreviewModel(id=PREVIEW_ID, benchmark_id=BENCHMARK_ID, document_id=DOCUMENT_ID,
        page_number=1, created_by=REVIEWER.subject_id)
    session.get.return_value = preview
    request = EvaluationReferenceSaveRequest(preview_id=PREVIEW_ID, expected_version=0, text="Human text",
        human_reviewed=True, compared_with_original=True, reason="Compared the original")
    ...
    async def scenario() -> None:
        ...
        with monkeypatch.context() as patch:
            patch.setattr(module, "load_material_source", AsyncMock(return_value=replace(SOURCE, page_count=None)))
        ...
        assert await service.image(BENCHMARK_ID, PREVIEW_ID, principal=REVIEWER) == b"verified image"
        ...
        with pytest.raises(module.EvaluationReferenceError, match="version_conflict"):
            await service.save(BENCHMARK_ID, DOCUMENT_ID, 1, request.model_copy(update={"expected_version": 9}), principal=REVIEWER)
        with pytest.raises(PageImageError, match="artifact_unavailable"):
        ...
        stored = next(call.args[0] for call in reversed(session.add.call_args_list) if isinstance(call.args[0], SourceEvaluationReferenceModel))
        ...
        for limit, offset in ((0, 0), (21, 0), (True, 0), (1, -1), (1, True)):
            with pytest.raises(ValueError, match="pagination"):
                await service.history(BENCHMARK_ID, DOCUMENT_ID, 1, principal=REVIEWER, limit=limit, offset=offset)
