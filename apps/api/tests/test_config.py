...
def test_pdf_size_ceiling_accepts_256_mib_without_raising_defaults() -> None:
    ...
    assert defaults.max_upload_bytes == 25 * 1024 * 1024
    ceiling = 256 * 1024 * 1024
    settings = Settings(environment="test", max_upload_bytes=ceiling)
    assert settings.max_upload_bytes == ceiling
    with pytest.raises(ValidationError):
        Settings.model_validate({"environment": "test", "max_upload_bytes": ceiling + 1})


def test_no_legacy_source_reader_settings_remain() -> None:
