...
from exam_guru_api.documents.page_images import PageImageArtifacts, PageImageError, load_material_source
...
from tests.integration.test_source_evaluation_references_postgres import failed_source, reference_client
...
def test_direct_reference_service_keeps_image_identity_and_version_boundaries(
    ...
    async def scenario() -> None:
        ...
        async with database_session(workspace_database_url) as session:
            ...
            with pytest.raises(module.EvaluationReferenceError, match="evaluation_page_not_selected"):
            ...
            with monkeypatch.context() as patch:
                patch.setattr(module, "load_material_source", AsyncMock(return_value=replace(identity, page_count=None)))
            ...
            evidence: dict[str, object] = {"page_image": stored.image_metadata, "source_checksum_sha256": identity.checksum_sha256}
            assert module._capture_image(identity, 1, evidence, storage, artifacts) == stored.image_metadata
            for invalid in (None, {}, {"failure_code": "unavailable"}, {"page_number": 2}):
                with pytest.raises(PageImageError, match="metadata_invalid"):
                    module._capture_image(identity, 1, {**evidence, "page_image": invalid}, storage, artifacts)
            request = EvaluationReferenceSaveRequest(preview_id=preview.id, expected_version=0, text="Human fixture reference",
                human_reviewed=True, compared_with_original=True, reason="Compared synthetic original")
            with pytest.raises(module.EvaluationReferenceError, match="preview_not_found"):
            ...
            second = await service.save(benchmark_id, document_id, 1,
                request.model_copy(update={"expected_version": 1, "text": "Revised human fixture reference"}), principal=REVIEWER)
            assert [entry.id for entry in await service.history(benchmark_id, document_id, 1, principal=REVIEWER)] == [second.id, first.id]
