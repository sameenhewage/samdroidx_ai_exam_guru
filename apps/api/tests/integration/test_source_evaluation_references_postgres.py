...
from exam_guru_api.documents.fidelity_models import PageGroundTruthModel, PageReviewEventModel, SourceEvaluationReferenceModel
...
from tests.integration.test_fidelity_workspace_postgres import (
    ADMIN, ADMIN_HEADERS, REVIEWER, REVIEWER_HEADERS, StaticIdentityProvider,
    database_session, workspace_database_url,
)
...
@pytest.fixture
def reference_client(workspace_database_url: str, tmp_path: Path) -> Iterator[tuple[TestClient, LocalFileObjectStorage]]:
...
async def failed_source(database_url: str, storage: LocalFileObjectStorage) -> tuple[UUID, UUID]:
    ...
    async with database_session(database_url) as session:
        session.add(SourceDocumentModel(
            id=document_id, checksum_sha256=checksum, object_key=key,
            original_filename="Synthetic evaluation reference fixture.pdf", content_type="application/pdf",
            size_bytes=len(content), document_type=SourceDocumentType.OTHER_APPROVED,
            original_page_count=1, intake_metadata=intake, metadata_review_required=True,
            created_by=ADMIN.subject_id, updated_by=ADMIN.subject_id,
        ))
        session.add(AdminAuditEventModel(
            id=uuid4(), actor_id=ADMIN.subject_id, resource_type="source_document", resource_id=document_id,
            action="source_document.uploaded", payload={"intake_metadata": intake, "metadata_review_required": True},
        ))
        await session.commit()
        service = PageFidelityService(session)
        await service.record_candidate(document_id, 1, raw_text="lvqZZaa;%%", method="native", actor_id=ADMIN.subject_id,
            provenance={"fonts": ["FMAbhaya"], "source_languages": ["si"], "languages": ["en"]})
        benchmark_id = await service.create_benchmark(name=f"Synthetic evaluation-only reference {document_id}",
            pages=((document_id, 1, ("blocked",)),), actor_id=ADMIN.subject_id,
            selection={"purpose": "disposable workflow test"})
    return document_id, benchmark_id


def test_empty_evaluation_migration_roundtrip_keeps_existing_source_tables(workspace_database_url: str) -> None:
    async def count_sources() -> int:
        async with database_session(workspace_database_url) as session:
            return int(await session.scalar(select(func.count()).select_from(SourceDocumentModel)) or 0)
...
def test_human_evaluation_reference_is_independent_of_source_confirmation(
    workspace_database_url: str, reference_client: tuple[TestClient, LocalFileObjectStorage],
) -> None:
    ...
    body = {"preview_id": preview["id"], "expected_version": 0, "text": "The original value is 4.",
        "blank_reference": False, "compared_with_original": True, "human_reviewed": True,
    ...
    confirmation = client.post(f"{PREFIX}/materials/{document_id}/pages/1/confirm", headers=ADMIN_HEADERS,
        json={"candidate_id": before["page"]["candidate_id"], "expected_version": before["page"]["version"],
              "compared_with_original": True, "reason": "Evaluation does not approve the source"})
    assert confirmation.status_code == 409
    assert client.post(path, headers=REVIEWER_HEADERS, json=body).status_code == 409
    assert client.get(workspace_path, headers=ADMIN_HEADERS).json() == before
    benchmark = client.get(f"{PREFIX}/source-benchmarks/{benchmark_id}", headers=REVIEWER_HEADERS).json()
    assert benchmark["adjudicated_pages"] == 0
    assert benchmark["pending_pages"] == 1
    assert benchmark["evaluation_references"] == {"selected_pages": 1, "referenced_pages": 1, "pending_pages": 0, "evaluation_only": True}
    ...
    async def check_gates() -> None:
        async with database_session(workspace_database_url) as session:
            ...
            assert await session.scalar(select(func.count()).select_from(PageReviewEventModel).where(
                PageReviewEventModel.document_id == document_id, PageReviewEventModel.action == "confirmed")) == 0
            assert await session.scalar(text("SELECT public.source_document_fidelity_is_current(:id)"), {"id": document_id}) is False
    asyncio.run(check_gates())


def reference_case(database_url: str, client: TestClient, storage: LocalFileObjectStorage) -> tuple[str, dict[str, object], dict[str, object]]:
...
def test_reference_versions_preserve_raw_unicode_and_remain_distinct_from_source_truth(
    ...
    for version, value in enumerate(("cafe\u0301", "ශ්‍රී ලංකාව")):
        saved = client.post(path, headers=REVIEWER_HEADERS, json={**body, "text": value, "expected_version": version})
        assert saved.status_code == 201, saved.text
    ...
    assert client.post(path, headers=ADMIN_HEADERS, json={**body, "expected_version": 2}).status_code == 404
...
@pytest.mark.parametrize("update", [
    {"compared_with_original": False}, {"compared_with_original": "true"}, {"human_reviewed": None},
    {"expected_version": True}, {"blank_reference": True}, {"text": ""}, {"text": "\t\n"},
    {"text": "private-reference-marker\x00"}, {"text": "x" * 100001}, {"can_confirm": True},
])
def test_reference_requires_explicit_bounded_human_input_without_echoing_rejected_text(
    workspace_database_url: str, reference_client: tuple[TestClient, LocalFileObjectStorage], update: dict[str, object],
) -> None:
    client, storage = reference_client
    page_path, _, body = reference_case(workspace_database_url, client, storage)
    response = client.post(f"{page_path}/evaluation-references", headers=REVIEWER_HEADERS, json={**body, **update})
...
def test_empty_reference_requires_explicit_blank_confirmation(
    ...
    client, storage = reference_client
    page_path, _, body = reference_case(workspace_database_url, client, storage)
    saved = client.post(f"{page_path}/evaluation-references", headers=REVIEWER_HEADERS, json={**body, "text": "", "blank_reference": True})
    assert saved.status_code == 201, saved.text
...
def test_evaluation_reference_evidence_is_append_only_and_cannot_be_downgraded_away(
    ...
    client, storage = reference_client
    page_path, preview, body = reference_case(workspace_database_url, client, storage)
    response = client.post(f"{page_path}/evaluation-references", headers=REVIEWER_HEADERS, json=body)
    assert response.status_code == 201
    identifier = response.json()["id"]
    async def tamper() -> None:
        async with database_session(workspace_database_url) as session:
            for statement, value in (
                ("UPDATE source_evaluation_references SET reason='changed' WHERE id=:id", identifier),
                ("DELETE FROM source_evaluation_references WHERE id=:id", identifier),
                ("UPDATE source_evaluation_previews SET image_metadata='{}'::jsonb WHERE id=:id", preview["id"]),
                ("DELETE FROM source_evaluation_previews WHERE id=:id", preview["id"]),
            ):
                with pytest.raises(DBAPIError, match="append only"):
                    await session.execute(text(statement), {"id": value})
                await session.rollback()
            for tables in ("source_evaluation_references", "source_evaluation_references, source_evaluation_previews"):
                with pytest.raises(DBAPIError, match="append only"):
    ...
    with pytest.raises(DBAPIError, match="cannot discard evaluation reference evidence"):
        command.downgrade(_config_for_database(workspace_database_url), "0041_source_metadata_candidates")
...
def test_concurrent_human_reference_saves_do_not_overwrite_a_revision(
    ...
    def save(identifier: object) -> int:
        return client.post(f"{page_path}/evaluation-references", headers=REVIEWER_HEADERS, json={**body, "preview_id": identifier}).status_code
...
def test_reference_write_serializes_with_source_identity_and_quarantine_changes(
    ...
    async def scenario() -> None:
        async with database_session(workspace_database_url) as first:
            ...
            first.add(SourceEvaluationReferenceModel(
                id=uuid4(), preview_id=UUID(str(preview["id"])), benchmark_id=UUID(str(preview["benchmark_id"])),
                document_id=UUID(str(preview["document_id"])), page_number=1, version=1, raw_text_utf8=raw,
                normalized_text=raw.decode(), text_sha256=digest, normalized_sha256=digest, blank_reference=False,
                reason="Source serialization test", reviewer_id=REVIEWER.subject_id,
            ))
            await first.flush()
            async with database_session(workspace_database_url) as second:
                ...
                with pytest.raises(DBAPIError, match="lock timeout"):
                    await second.execute(text("UPDATE source_documents SET updated_at=updated_at WHERE id=:id"), {"id": preview["document_id"]})
...
def test_reference_save_rechecks_the_original_not_just_the_preview(
    workspace_database_url: str, reference_client: tuple[TestClient, LocalFileObjectStorage], tmp_path: Path,
) -> None:
    client, storage = reference_client
    page_path, _, body = reference_case(workspace_database_url, client, storage)
...
def test_reference_image_loss_fails_closed_without_regeneration_or_source_mutation(
    ...
    client, storage = reference_client
    page_path, preview, body = reference_case(workspace_database_url, client, storage)
    artifact = next(path for path in (tmp_path / "originals" / "fidelity-page-images").rglob("*") if path.is_file())
